    ...
    <div className="border border-border rounded-lg p-5 bg-surface" {...containerProps}>
      <svg viewBox="0 0 360 200" className="w-full" style={{ maxHeight: 220 }}>
        ...
        <text
          ...
          fontSize={8} fontFamily="monospace" letterSpacing={2} opacity={0.6}
        >
          CLUSTER
        </text>
        {CLUSTER_SERVERS.map((cx, i) => (
          <g key={i}>
            ...
            <text
              ...
              fill={isDisconnected ? COLORS.textTertiary : COLORS.green} fontSize={7} fontFamily="monospace"
            >
              n-{i + 1}
            </text>
          </g>
        ...
        {LEAF_NODES.map((leaf, i) => {
          const lx = LEAF_XS[i];
          // Connection line goes from bottom of cluster to top of leaf box
          const lineTop = 63;
          const lineBottom = LEAF_Y - 8;
          // Angle lines to connect to the nearest cluster server
            ...
            <g key={leaf.name}>
              ...
              {isDisconnected && (
                <motion.text
                  ...
                  fill={COLORS.red} fontSize={11} fontWeight="bold"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                >
                  ✕
              ...
              <text
                ...
                fill={leaf.color} fontSize={9} fontWeight={500}
              >
                {leaf.name}
              </text>
              <text
                x={lx} y={LEAF_Y + 20} textAnchor="middle" dominantBaseline="middle"
                fill="var(--text-quaternary)" fontSize={7} fontFamily="monospace"
              >
                {leaf.subject}
              </text>
